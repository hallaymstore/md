...
app.use('/static', express.static(path.join(__dirname, 'public'), { etag:false,lastModified:false,maxAge:0,setHeaders:(res)=>res.setHeader('Cache-Control','no-store, no-cache, must-revalidate') }));
...
app.use('/display',displayRoutes);app.use('/md',mdRoutes);app.use(authRoutes);app.use('/dashboard',dashboardRoutes);app.use('/profile',profileRoutes);app.use('/users',userRoutes);app.use('/students',studentRoutes);app.use('/monitoring',monitoringRoutes);app.use('/uploads',uploadRoutes);app.use('/audit',auditRoutes);app.use('/analytics',analyticsRoutes);app.use('/structure',structureRoutes);app.use('/control',controlRoutes);app.use('/submissions',submissionRoutes);app.use('/notifications',notificationRoutes);app.use('/insights',insightsRoutes);app.use('/milestones',milestoneRoutes);app.use('/integrations/hemis',hemisRoutes);app.use('/timetable',timetableRoutes);app.use(modulesRoutes);
app.use((req,res)=>res.status(404).render('errors/404',{title:'Sahifa topilmadi'}));
app.use((err,req,res,next)=>{console.error(err);if(err?.name==='MulterError'||/fayl turi|file too large|xlsx excel/i.test(err?.message||'')){let redirectTo=req.originalUrl?.startsWith('/submissions')?'/submissions':'/uploads';if(req.originalUrl?.startsWith('/timetable/import'))redirectTo='/timetable/import';if(req.originalUrl?.startsWith('/profile/avatar'))redirectTo='/profile/edit';if(req.originalUrl==='/submissions')redirectTo='/submissions/new';const match=req.originalUrl?.match(/^\/submissions\/([a-f0-9]{24})\//i);if(match)redirectTo=`/submissions/${match[1]}`;const text=err.code==='LIMIT_FILE_SIZE'?(req.originalUrl?.startsWith('/timetable/import')?'Excel fayli 8 MB limitdan oshgan.':req.originalUrl?.startsWith('/profile/avatar')?'Profil rasmi 3 MB limitdan oshgan.':`Fayl hajmi ${process.env.SUBMISSION_FILE_LIMIT_MB||process.env.UPLOAD_LIMIT_MB||25} MB limitdan oshgan.`):err.code==='LIMIT_FILE_COUNT'?`Bir urinishda ko‘pi bilan ${process.env.SUBMISSION_MAX_FILES||5} ta fayl yuklang.`:(err.message||'Faylni yuklashda xato yuz berdi.');req.session.flash={type:'error',text};return res.redirect(redirectTo);}res.status(err.status||500).render('errors/500',{title:'Server xatosi',error:process.env.NODE_ENV==='development'?err:null});});
module.exports = app;
